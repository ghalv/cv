...
\begin{itemize}
\item Har bakgrunn innen matematikk/statistikk, men ser i grunnen på meg selv som en datatype. Interesser innen modellering, analyse og GNU/linux-systemer. Inngående kjennskap til verktøy som {\LaTeX}, git, R, og da spesielt 'tidyverse-dialekten'. Aktiv bruker av shell og vim, og har erfaring med python og teknologier som GraphQL, docker, ansible, html og css fra ulike prosjekter.

\item Erfaring med drift av servere, bl.a. VPS med Debian 11 for nettside (nginx) og epost-server (postfix), som har tilfredsstilt 'five nines' over flere år. God kjennskap til Windows og MacOS, samt programvare som tmux, ssh, RStudio, pandoc og Jupyter, for å nevne noen. Har også begynt å sette meg inn i DevOps, og pensum til AZ-900.
...
\entry{2012--2013}
        ...
        {Sandnes Kommune}
        {Jobbet ved Soma og Stangeland skole. Hadde bl.a. en 7. klasse med på Knaben leirskole.}
...
\begin{itemize}
\item Teknisk løsning til forskningsprosjektet SPEDAIMS som involverte en android app (kotlin) som eksporterte data til tsv, som ble formatert til tex via bash-skript, og eksportert som pdf-dokument.

\item Shellskript som sender kommandoer til Stihl robotgressklipper via API.

\item Front-end for API-data fra netatmo-værstasjon.

\item Patchet driver til wifi-kort i tråd med endringer i Linux-kjerne 5.16. Lite inngripende endring.
